...
(function ($) {
  ...
  $.fn.pixelPicker = function(options) {
    ...
    applyColor = function(cell) {
      var classCount = $(cell).attr('data-classCount');
      var cellsClass = 'newCellClass'+classCount;
      var exactColor = '';
      var selector = '';
      if($('#eyeDropper').hasClass('isClicked')){
          exactColor = $(cell).css('background-color');
          selector = $(cell);
      }else{
          exactColor = arrayToRgb(currentColor);
          selector = $('.pixel-picker-cell[data-classCount="'+classCount+'"]');
          var cells = [];
          selector.each(function(){
            cells.push({'cell':this,'color':$(this).css('background-color')});
          });
          stackEvents.push(cells);
          $('#undo').removeAttr('disabled');
          undoEvents = [];
          $('#redo').attr('disabled','disabled');
      }
      return selector.css('background-color', exactColor);
    };
    ...
    rows.each(function(rowIndex, row) {
      ...
      cells.each(function(cellIndex, cell) {
        ...
        // When a cell is clicked in to...
         pixelMouseDown = cell.on('mousedown', function(event) {
        ...
        // Turn dragging off when we mouse up
           pixelMouseUp = cell.on('mouseup', function() {
          isDragging = false;
        });
        ...
        // global color to the cell, no cycling
           pixelMouseEnter = cell.on('mouseenter', function() {
